...
write_csv(table3, path = "Output/Table4.csv")
```
...
### Figure S2
Create a subset of F1 testing results for the final models (Runs 10 for FPN2-WS and 37 for MRCNN) and comparison with the out-of-the-box Jacobkie model (Run 44) using either the F1 training or testing inference results (Fig. S3).
```{r filter-FigS2}
fig_S3 <- runs_tbl %>%
...
Generate and save a table for the F1 results shown in Fig S3 at the 0.7 and 0.9 thresholds.
```{r}
tableS3 <- fig_S3 %>%
  filter(round(thres, digits = 2) %in% c(0.70, 0.9)) %>%
