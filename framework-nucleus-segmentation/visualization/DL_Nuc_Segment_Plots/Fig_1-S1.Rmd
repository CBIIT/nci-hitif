---
title: "Generation of Figures 1 to S1 for the DL Nuclear Segmentation Manuscript"
date: "Jun 19 2020"
...
Generate and save a table for the F1 results shown in Fig 2 at the 0.7 and 0.9 thresholds.
```{r table1}
table1 <- fig_2B %>%
...
fig_5B_plot + geom_line() +
  ...
  ylab("F1 Score")
```
Plot the performance for the final MRCNN and FPN2-WS models comparing it with Jacobkie (Run044) using both semi-automated generated labels and manually annotated labels. This is Fig. S1A.
```{r manualSet}
...
```{r FigS1A, echo = F}
fig_S1A_plot <- ggplot(fig_S1A_set, aes(x = thres, 
                                        y = F1,
                                        color = annotation))

fig_S1A_plot + geom_line() +
  geom_vline(xintercept = 0.7, 
             linetype = "longdash",
             color = "grey60") +
  facet_grid(cell_line ~ model) +
  scale_color_few(name= "Annotation") +
  xlab("IoU Threshold") +
  ylab("F1 Score")
```
Generate and save a table for the F1 results shown in Fig 5B at the 0.7 and 0.9 thresholds.
```{r table2}
table2 <- fig_5B_set %>%
  filter(round(thres, digits = 2) %in% c(0.70, 0.9)) %>%
  select(Model = model, 
