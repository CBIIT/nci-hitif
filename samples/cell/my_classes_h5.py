...
class DataGeneratorH5(keras.utils.Sequence):
    ...
    def __init__(self, source_target_list_IDs, h5fname, source_datasetname, target_datasetname, batch_size=16, dim=(256,256), n_in_channels=3,n_out_channels=1, shuffle=True, scale_source=True, scale_target=True):
        'Initialization'
        self.dim = dim
    ...
    def __data_generation(self, src_tar_list_IDs_temp):
        ...
        with h5py.File(self.h5fname, 'r') as hf:
             ...
             for j in sorted_list:
                X[i,:,:,0] = hf[self.source_datasetname][j].astype('float32')
                X[i,:,:,1] = hf[self.source_datasetname][j].astype('float32')
                X[i,:,:,2] = hf[self.source_datasetname][j].astype('float32')               
                Y[i,:,:,0] = hf[self.target_datasetname][j]
                i = i+1
